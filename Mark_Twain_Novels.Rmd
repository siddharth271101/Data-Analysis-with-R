...
We got the data, the next step is tokenization.Where we split the data into individual words using the `unnest_tokens` function.Then remove stop words using `antijoin` function. 
...
Earlier we simply counted the occurence of each word and listed the most frequent words.One of the problems with this approach is that a word’s context can matter nearly as much as its presence.Thus we will now work with tokens containing two (bigrams) or three(trigrams) words as it can help to extract useful phrases which leads to some additional insights.
...
In the below steps, we first split the bigrams into two. Remove if any of the word is a stopword and then we unite them back again. 
...
After filtering the next step is essentially to find the most common bigrams in each of the novels.
...
In the word network we can see that quantitative words like "yards","miles","dollars","feet" all have a common node "hundred".Also we can see salutation such as "aunt","miss","col"  are followed by their names.Other than those we can see a lot of names coming up.
...
To verify this we will use the bigrams which we collected earlier and find the most common words which are preceded by negation.
...
Next will look how the sentiments were distributed for each of the four novels.Since "The Glided Age" and "Roughing It" are relatively biggers novels in terms of number of words,I have divided each words contribution to the sentiment of a novel by the total contribution to that novel.So that all the novels are in similar scale.
